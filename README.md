...
`자비스 깨어나`로 활성화되면 30초 동안 명령 대기 상태가 됩니다. `카메라 실행`, `후면`, `전면`, `찍어`, `종료` 같은 명령을 처리한 뒤에도 30초 command window를 다시 엽니다. 30초 안에 다음 명령이 없으면 Jarvis는 실패음 없이 조용히 idle activation 대기로 돌아가고, overlay도 사라집니다. 로컬 ASR이나 Android STT 재시도는 이 시간을 넘겨 명령 대기 상태를 연장하지 않습니다. command window가 닫힌 뒤 예약되어 있던 다음 STT 시작은 폐기됩니다.

Idle에서는 Android 기본 `SpeechRecognizer` wake 보조 경로가 먼저 `자비스 깨어나`만 듣습니다. 기본 recognizer 생성이 실패할 때만 Android System Intelligence(AiAi) recognizer로 fallback합니다. 앱 내부 `IdleWakeAudioBuffer`는 Android STT의 `ready_for_speech` 이후 최근 4.5초 마이크 오디오를 보존하고, Android STT가 activation phrase를 잡은 경우에도 이 오디오로 3D-Speaker owner voice verification을 통과해야 command window를 엽니다. Android STT wake가 `ERROR_CLIENT`/`ERROR_RECOGNIZER_BUSY`처럼 시작 에러를 3회 연속 반환하거나, 실제 발화를 감지한 뒤 `NO_MATCH`/timeout을 반환하면 해당 snapshot을 즉시 local activation ASR로 replay합니다. replay도 실패하면 60초 동안 Android wake를 쉬고 sherpa-onnx local activation ASR로 fallback합니다. wake 보조 STT는 idle 명령 실행에는 사용하지 않습니다.

Local activation fallback에서는 sherpa-onnx 한국어 streaming ASR의 activation 전용 hotwords recognizer가 `자비스 깨어나`를 듣습니다. 마이크 `AudioRecord`는 `VOICE_RECOGNITION` source를 우선 사용하고 실패 시 `MIC`로 fallback하며, 60초 세션으로 길게 유지하고 ASR stream은 최대 8초 segment 단위로만 재시작합니다. 이 때문에 Android/HyperOS 우상단의 초록 마이크 privacy chip과 `JARVIS` 앱명 표시는 Jarvis가 idle 대기 중 마이크를 사용한다는 OS 표시이며, 앱이 숨길 수 없습니다. Jarvis 자체 overlay는 화면 상단 중앙의 작은 `JARVIS` pill이고, activation phrase와 owner voice가 모두 통과해 command window가 열린 경우에만 표시됩니다.

activation recognizer는 `modified_beam_search`, `jarvis-activation-hotwords.txt`, `modelingUnit=cjkchar`를 사용하며, 최근 3.6초 rolling audio를 0.8초마다 buffered hotword decode로 다시 확인합니다. streaming final이 `깨우나`처럼 일부만 잡혀도 같은 rolling audio의 buffered decode에서 `자비스 깨어나` 계열이 확인되면 activation으로 처리합니다. 문구가 확인되면 그때 보존된 최근 오디오로 3D-Speaker owner voice verification을 실행하고, 소유자 목소리까지 통과한 경우에만 Android `SpeechRecognizer` 명령 리스닝을 시작합니다. activation owner verification은 긴 rolling audio 전체 점수와 함께 1.2초, 1.6초, 2.2초 최고 에너지 후보 구간을 비교해 가장 높은 owner score를 사용하고, replay 로그에는 `ownerFullScore`, `ownerWindowStartMs`, `ownerWindowMs`, `ownerWindows`를 함께 남깁니다. activation 문구가 아니거나 소유자 목소리가 아니면 overlay, 준비음, Android command STT 없이 조용히 idle로 돌아갑니다. Android STT가 실제 발화를 감지한 뒤 실패했거나 사용할 수 없을 때만 local ASR fallback을 사용하고, 아무 발화도 감지하지 못한 `NO_MATCH`/timeout은 실패 피드백 없이 command window 안에서 조용히 다시 듣습니다. local ASR fallback은 `0.0012` RMS 이상의 말소리를 기준으로 잡고, activation ASR은 `0.00022` RMS 이상의 입력을 segment 대상으로 봅니다. activation ASR은 최소 560ms active speech와 600ms trailing silence 조건을 만족해야 segment를 끊어 짧은 파편에 너무 빨리 endpoint가 잡히지 않게 합니다. local ASR fallback은 최소 청취 560ms 이후 240ms trailing silence가 감지되면 빠르게 final decode로 넘어갑니다. 입력 음량이 낮아 ASR 텍스트가 비는 문제를 줄이기 위해 원본 RMS가 `0.00008` 이상인 구간은 ASR 입력에서 목표 RMS `0.04`, 최대 `30x`까지 증폭합니다. 명령 처리 직후에는 80ms 뒤에 다음 명령 리스닝을 시작합니다. `종료`, `홈`, `뒤로`는 현재 앱만 제어하고 Jarvis 명령 대기 상태는 유지합니다. `자비스 잠들어`와 `멈춰`는 현재 명령 대기만 닫고 idle activation 대기로 돌아갑니다. Jarvis 서비스는 한 번 시작되면 재부팅 전까지 foreground service로 유지하는 것을 원칙으로 하며, 접근성 서비스가 켜져 있고 소유자 목소리와 마이크 권한이 준비되어 있으면 watchdog이 꺼진 음성 서비스를 다시 시작합니다.

ASR이 `자비스 깨어나`를 빈 문자열이나 `응.`으로 놓치는 경우를 보완하기 위해, 등록 시 저장된 `jarvis-owner-enroll-last.wav`에서 개인 wake phrase 템플릿을 추출해 envelope-DTW fallback을 수행합니다. 이 fallback은 owner enrollment 검증에는 사용하지 않고 idle activation/replay에만 사용합니다. template distance `0.27` 이하, candidate peak RMS `0.0012` 이상, owner voice near score `0.27` 이상이 모두 맞아야 command window 후보가 됩니다. owner verification은 full rolling audio와 template 후보 구간을 모두 평가하고 더 높은 score를 사용합니다. template 후보 구간은 짧게 잘릴 수 있으므로, activation phrase가 이미 확인된 경우에만 owner active speech 최소 길이를 250ms, owner peak RMS 최소값을 `0.0012`로 둡니다.
...
`jarvis-wake-live-check.sh`는 실기기에서 wake만 검증합니다. debug APK에서는 기본적으로 no-display debug Activity로 음성 서비스를 재시작해 activation 세션 시작점과 측정 시작점을 맞춥니다. 실행하면 폰이 짧게 진동하고, 사용자는 진동 직후 `자비스 깨어나`를 말합니다. 스크립트는 `android_activation_detected`, `android_activation_audio_snapshot`, `android_activation_disabled`, `android_activation_local_replay_complete`, `android_activation_local_replay_detected`, `activation_partial`, `owner_audio_activation`, `ready_for_speech`, `JarvisStateIndicator overlay_visible state=COMMAND_READY`를 함께 확인해 상단 중앙 `JARVIS` overlay가 실제로 표시됐는지 판정합니다. `jarvis-wake-diagnose.sh`는 프로필 상태, 저장된 activation WAV replay, 최신 캡처 메타데이터, 관련 logcat을 한 번에 묶어 wake 실패 원인을 먼저 분류합니다. `jarvis-activation-replay.sh`는 저장된 WAV들을 앱 내부에서 재디코딩해 accepted/text/RMS와 owner score를 로그로 요약합니다. `jarvis-activation-captures.sh`는 WAV/JSON 묶음을 `/tmp`로 가져와 사람이 직접 확인하거나 별도 분석에 사용할 수 있게 합니다. 이 흐름으로 ASR rule과 threshold 변경은 저장 샘플로 먼저 검증하고, 사용자의 실시간 발화는 최종 확인 단계에서만 요청합니다.

2026-06-21 Xiaomi 15 Ultra 화면 켠 상태의 실기기 확인에서도 Android STT wake는 실제 발화를 감지한 뒤 `NO_MATCH`를 반환했다. 이 snapshot은 485ms 안에 local activation replay까지 실행됐지만 accepted=0이었다. 이후 Android wake를 60초 쉬게 하고 local activation 세션으로 fallback되는 것은 확인했으나, 저장 WAV replay는 여전히 accepted=0이고 상단 중앙 `JARVIS` overlay도 확인되지 않았다. 따라서 `자비스 깨어나` live wake 성공은 아직 확정되지 않았다.
...
소유자 목소리 인증은 오픈소스 `sherpa-onnx` 런타임과 3D-Speaker CAM++ ONNX 모델을 사용합니다. 앱에 등록된 소유자 embedding이 있으면 Jarvis는 먼저 local activation ASR로 `자비스 깨어나`를 찾고, activation이 확인된 rolling audio를 speaker embedding 입력으로 사용합니다. speaker embedding 입력은 내부에서 최소 1.2초로 padding합니다. 등록 시에는 `자비스 깨어나`를 반복한 전체 6초 음성을 먼저 activation hotwords ASR로 확인하고, activation phrase가 확인된 경우에만 1.4초 단위 짧은 구간에서 최소 2개, 최대 8개의 embedding을 저장하며 `profile_phrase_id=jarvis_activation_v3` 메타데이터를 함께 저장합니다. verification은 등록 embedding 묶음 중 가장 높은 similarity를 사용하고, activation phrase 확인 뒤에는 전체 rolling audio와 1.2초/1.6초/2.2초 최고 에너지 후보 구간의 owner score 중 가장 높은 값을 사용합니다. 기본 threshold `0.50`을 넘으면 strict 통과이고, activation phrase가 이미 확인된 경우에만 다중 embedding 프로필에서 active speech 450ms 이상, similarity `0.36` 이상 고신뢰 단일 점수 또는 `0.28` 이상 근접 점수를 통과시킵니다. 이 activation-after-owner 경로는 peak RMS `0.0035` 이상일 때만 허용하며, 기존 owner gate의 soft wake 단일/연속 점수는 command window 오픈 조건으로 사용하지 않습니다.

2026-06-21 16:45 KST Xiaomi 15 Ultra 화면 켠 상태 live check에서는 Android STT wake가 발화 시작/끝과 4.36초 snapshot을 잡았지만 `NO_MATCH`를 반환했고, snapshot local replay도 text가 비어 accepted=0이었습니다. 이어진 local activation session은 `응.`, `진자` 같은 조각만 반환했으며 `자비스 깨어나` activation equivalent로 통과하지 못했습니다. 새 windowed owner replay에서는 일부 저장 캡처의 owner score가 near 기준까지 올라가는 것을 확인했지만, activation ASR accepted가 0이라 command window와 상단 중앙 `JARVIS` overlay는 아직 열리지 않았습니다.

2026-06-21 17:09 KST 저장 캡처 replay에서 template fallback과 owner verification이 함께 통과한 캡처가 확인됐습니다(`endpoint=debug_activation_replay_template`, ownerScore `0.2835`). 그러나 17:11 KST 60초 live check에서는 Android snapshot replay의 template distance가 `0.319`로 높았고, local activation fallback도 `자비스 깨어나`를 안정적으로 만들지 못해 상단 중앙 `JARVIS` overlay는 아직 확인되지 않았습니다. 17:15 KST 화면 켠 상태 추가 live/replay에서는 template phrase match 5건이 있었으나 owner active speech가 275ms처럼 짧게 잘린 케이스가 거절되어, activation phrase owner 최소 active speech를 250ms로 조정했습니다. 17:22 KST live check에서는 `자비스 깨어나`가 인식되고 owner score `0.278`로 near 기준을 넘었지만 peak RMS `0.00123`이 기존 보조 peak 조건보다 낮아 거절되어, activation phrase owner peak 최소값을 `0.0012`로 조정했습니다. 17:26 KST replay는 `accepted=1`, `phraseAccepted=5`로 문구+소유자 동시 통과를 확인했습니다. 다만 이어진 17:27 KST live check에서는 다시 문구 인식 자체가 실패해 overlay는 확인되지 않았습니다. 따라서 wake 성공은 아직 완료 기준을 만족하지 않습니다.

중요한 점은 activation phrase와 owner voice가 둘 다 확인되어야 command window가 열린다는 것입니다. Android STT wake 보조 경로 또는 local activation ASR이 `자비스 깨어나` 계열을 먼저 확인하고, 같은 시점에 보존된 오디오의 owner score가 통과한 경우에만 30초 command window를 열고 초록 `JARVIS` overlay와 준비음/진동을 제공합니다. `자비스` 단독, `헤이 자비스 깨어나`, `자비스 카메라 실행`, 주변 대화, 소유자의 다른 말은 idle 상태에서 command window를 열지 않습니다. 이후 live command window는 Android 기본 `SpeechRecognizer`를 먼저 사용하고, Android STT가 실패하거나 사용할 수 없을 때만 local ASR fallback을 사용합니다. 로컬 ASR fallback은 짧은 명령 후 trailing silence를 감지하면 timeout 전에도 final decode를 실행합니다. command window는 서비스 레벨의 30초 deadline으로 닫히므로 fallback 재시도 때문에 무한히 유지되지 않습니다. 이 때문에 Jarvis 대기 중에는 Android의 초록색 마이크 표시가 켜져 있는 것이 정상입니다.
