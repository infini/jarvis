...
Idle에서는 sherpa-onnx local activation ASR과 개인 wake phrase template matcher가 먼저 `자비스 깨어나`만 듣습니다. 이 경로는 60초 `AudioRecord` 세션을 길게 유지하므로 Android/HyperOS 우상단의 초록 마이크 privacy chip이 깜빡이는 대신 켜진 상태로 유지되는 것이 정상입니다. local activation을 시작할 수 없을 때만 Android 기본 `SpeechRecognizer` wake 보조 경로를 사용합니다. wake 보조 STT는 idle 명령 실행에는 사용하지 않습니다.

Local activation에서는 sherpa-onnx 한국어 streaming ASR의 activation 전용 hotwords recognizer가 `자비스 깨어나`를 듣습니다. 마이크 `AudioRecord`는 `VOICE_RECOGNITION` source를 우선 사용하고 실패 시 `MIC`로 fallback하며, 60초 세션으로 길게 유지하고 ASR stream은 최대 8초 segment 단위로만 재시작합니다. 이 때문에 Android/HyperOS 우상단의 초록 마이크 privacy chip과 `JARVIS` 앱명 표시는 Jarvis가 idle 대기 중 마이크를 사용한다는 OS 표시이며, 앱이 숨길 수 없습니다. Jarvis 자체 overlay는 화면 상단 중앙의 작은 `JARVIS` pill이고, activation phrase와 owner voice가 모두 통과해 command window가 열린 경우에만 표시됩니다.
...
scripts/jarvis-owner-calibrate-from-wake-captures.sh
```

`jarvis-wake-live-check.sh`는 실기기에서 wake만 검증합니다. debug APK에서는 기본적으로 no-display debug Activity로 음성 서비스를 재시작해 activation 세션 시작점과 측정 시작점을 맞춥니다. 실행하면 폰이 짧게 진동하고, 사용자는 진동 직후 `자비스 깨어나`를 말합니다. 스크립트는 `activation_asr_complete`, `activation_owner_verified`, `owner_audio_activation`, `ready_for_speech`, `JarvisStateIndicator overlay_visible state=COMMAND_READY`를 함께 확인해 상단 중앙 `JARVIS` overlay가 실제로 표시됐는지 판정합니다. `jarvis-wake-diagnose.sh`는 프로필 상태, 저장된 activation WAV replay, 최신 캡처 메타데이터, 관련 logcat을 한 번에 묶어 wake 실패 원인을 먼저 분류합니다. `jarvis-activation-replay.sh`는 저장된 WAV들을 앱 내부에서 재디코딩해 accepted/text/RMS와 owner score를 로그로 요약합니다. `jarvis-owner-calibrate-from-wake-captures.sh`는 최근 activation phrase accepted 캡처만 골라 기존 owner profile에 embedding을 병합합니다. `jarvis-activation-captures.sh`는 WAV/JSON 묶음을 `/tmp`로 가져와 사람이 직접 확인하거나 별도 분석에 사용할 수 있게 합니다. 이 흐름으로 ASR rule과 threshold 변경은 저장 샘플로 먼저 검증하고, 사용자의 실시간 발화는 최종 확인 단계에서만 요청합니다.

2026-06-21 17:33 KST Xiaomi 15 Ultra 화면 켠 상태 live check에서 local activation/template 경로가 `자비스 깨어나`를 인식했고, owner verification은 `STRICT score=0.7506`으로 통과했습니다. 이어서 `owner_audio_activation`, Android command STT `ready_for_speech`, `JarvisStateIndicator overlay_visible state=COMMAND_READY`가 발생해 상단 중앙 `JARVIS` overlay 표시까지 확인했습니다.
...
소유자 목소리 인증은 오픈소스 `sherpa-onnx` 런타임과 3D-Speaker CAM++ ONNX 모델을 사용합니다. 앱에 등록된 소유자 embedding이 있으면 Jarvis는 먼저 local activation ASR로 `자비스 깨어나`를 찾고, activation이 확인된 rolling audio를 speaker embedding 입력으로 사용합니다. speaker embedding 입력은 내부에서 최소 1.2초로 padding합니다. 등록 시에는 `자비스 깨어나`를 반복한 전체 6초 음성을 먼저 activation hotwords ASR로 확인하고, activation phrase가 확인된 경우에만 1.4초 단위 짧은 구간에서 최소 2개, 최대 16개의 embedding을 저장하며 `profile_phrase_id=jarvis_activation_v3` 메타데이터를 함께 저장합니다. debug APK에서는 최근 accepted wake 캡처를 owner profile에 병합해 실제 폰 거리/음량의 발화를 보정할 수 있습니다. verification은 등록 embedding 묶음 중 가장 높은 similarity를 사용하고, activation phrase 확인 뒤에는 전체 rolling audio와 1.2초/1.6초/2.2초 최고 에너지 후보 구간의 owner score 중 가장 높은 값을 사용합니다. 기본 threshold `0.50`을 넘으면 strict 통과이고, activation phrase가 이미 확인된 경우에만 다중 embedding 프로필에서 active speech 450ms 이상, similarity `0.36` 이상 고신뢰 단일 점수 또는 `0.27` 이상 근접 점수를 통과시킵니다. activation phrase owner 보조 통과는 최소 active speech 250ms, peak RMS `0.0012` 이상일 때만 허용하며, 기존 owner gate의 soft wake 단일/연속 점수는 command window 오픈 조건으로 사용하지 않습니다.
...
2026-06-21 17:09 KST 저장 캡처 replay에서 template fallback과 owner verification이 함께 통과한 캡처가 확인됐습니다(`endpoint=debug_activation_replay_template`, ownerScore `0.2835`). 17:31 KST에는 최근 live wake 캡처를 owner profile에 병합해 embedding을 8개에서 10개로 보정했습니다. 17:33 KST live check에서는 `activation_asr_complete endpoint=rolling_buffer_live_buffered_template text=자비스 깨어나`, `activation_owner_verified accepted=true acceptance=STRICT score=0.7506`, `owner_audio_activation`, `overlay_visible state=COMMAND_READY`가 이어져 상단 중앙 `JARVIS` overlay 표시까지 통과했습니다.
