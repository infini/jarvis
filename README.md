...
Idle에서는 sherpa-onnx 한국어 streaming ASR의 activation 전용 hotwords recognizer가 먼저 `자비스 깨어나`를 듣습니다. 마이크 `AudioRecord`는 60초 세션으로 길게 유지하고, ASR stream은 최대 8초 segment 단위로만 재시작합니다. 이 때문에 Android/HyperOS 우상단의 초록 마이크 privacy chip과 `JARVIS` 앱명 표시는 Jarvis가 idle 대기 중 마이크를 사용한다는 OS 표시이며, 앱이 숨길 수 없습니다. Jarvis 자체 overlay는 화면 상단 중앙의 작은 `JARVIS` pill이고, activation phrase와 owner voice가 모두 통과해 command window가 열린 경우에만 표시됩니다.
