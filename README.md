...
Jarvis 상태 overlay는 사용자가 바로 판단해야 하는 순간에만 표시됩니다. 초록색 `JARVIS 듣는 중`은 명령 가능 상태이고, 파란색 `다음 명령 가능`은 명령 처리 후 이어서 말할 수 있는 상태이며, 빨간색 `다시 말하세요`는 인식 실패 후 재시도 상태입니다. idle/소유자 확인/호출어 대기 상태에서는 화면을 가리지 않도록 overlay를 숨깁니다. 명령 가능 상태에 들어갈 때는 확인음 2회와 짧은 진동이 함께 발생합니다.
