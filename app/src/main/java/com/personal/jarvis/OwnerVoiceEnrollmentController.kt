...
class OwnerVoiceEnrollmentController(
    ...
    fun start(durationMs: Long) {
        ...
        enrolling = true
        onProgress(0)
        onStatus("목소리 등록 중: 조용한 곳에서 6초 동안 자연스럽게 말하세요.")
        ...
        thread = Thread({
            try {
                ...
                val embedding = OwnerVoiceEngine.createEmbedding(context, samples)
                    ?: throw IllegalStateException("충분한 음성 특징을 만들지 못했습니다. 더 또렷하게 다시 등록하세요.")

                if (!enrolling) return@Thread

                OwnerVoiceStore.saveEmbedding(context, embedding)
                postToMain {
                    if (enrolling) {
                        enrolling = false
                        thread = null
