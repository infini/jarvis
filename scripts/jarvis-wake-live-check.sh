...
ANDROID_ACTIVATION_LOCAL_REPLAY_COUNT="$(count_event android_activation_local_replay_complete)"
ANDROID_ACTIVATION_LOCAL_REPLAY_DETECTED_COUNT="$(count_event android_activation_local_replay_detected)"
...
echo "summary activation_asr_complete=$ACTIVATION_COMPLETE_COUNT activation_partial=$ACTIVATION_PARTIAL_COUNT activation_asr_rejected_segment=$ACTIVATION_REJECTED_SEGMENT_COUNT android_activation_detected=$ANDROID_ACTIVATION_DETECTED_COUNT android_activation_audio_snapshot=$ANDROID_ACTIVATION_AUDIO_COUNT android_activation_disabled=$ANDROID_ACTIVATION_DISABLED_COUNT android_activation_local_replay=$ANDROID_ACTIVATION_LOCAL_REPLAY_COUNT android_activation_local_replay_detected=$ANDROID_ACTIVATION_LOCAL_REPLAY_DETECTED_COUNT activation_owner_verified=$ACTIVATION_OWNER_VERIFIED_COUNT owner_audio_activation=$OWNER_AUDIO_ACTIVATION_COUNT ready_for_speech=$READY_FOR_SPEECH_COUNT command_ready_feedback=$COMMAND_READY_FEEDBACK_COUNT overlay_ready=$OVERLAY_READY_COUNT activation_owner_rejected=$OWNER_REJECTED_COUNT activation_phrase_missing=$PHRASE_MISSING_COUNT"
echo "audio max_peak_rms=$MAX_PEAK_RMS max_speech_ms=$MAX_SPEECH_MS"

echo "recent_activation_events:"
grep -E "event=(activation_partial|activation_asr_complete|activation_asr_rejected_segment|android_activation_detected|android_activation_audio_snapshot|android_activation_disabled|android_activation_local_replay_start|android_activation_local_replay_complete|android_activation_local_replay_detected|activation_owner_verified|owner_audio_activation|activation_owner_rejected|activation_phrase_missing|ready_for_speech)" "$LOG_FILE" | tail -20 || true
