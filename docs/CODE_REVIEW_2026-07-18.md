...
2026-07-18 실행 결과는 단위 테스트 89개(실패/오류 0), debug APK 조립, Android lint, shell syntax, `git diff --check` 모두 통과다. debug APK는 `app/build/outputs/apk/debug/app-debug.apk`에 생성됐다.
...
4. ASR 모델 다운로드는 mutable revision과 파일 크기만 검증한다. immutable revision과 SHA-256 고정이 필요하다.
