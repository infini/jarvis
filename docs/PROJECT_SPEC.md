...
`scripts/jarvis-photo-live-check.sh`는 카메라 앱을 연 뒤 debug command window를 시작하고 사용자가 `자비스 사진 찍어`를 한 번 말하는 실발화 검증용 스크립트다. `partial_results`, `partial_no_command`, `final_results`, `parse_no_command`의 후보 목록과 사진 명령 진단, `command_parsed command=take_photo`, `parsed_candidate_index`, `stt_bias_count`, STT endpoint timing, `accessibility_command_received`, 셔터 좌표 fast path 로그를 함께 확인한다. `photo=` 진단에서 `missing_wake`는 호출어 오인식, `missing_shot`은 촬영 동사 누락, `missing_photo_or_direct_shot`은 사진 문맥과 direct 촬영 동사가 모두 부족한 후보로 보고 다음 bias/partial rule 후보를 정한다. 현재 debug 기준은 `JARVIS_PHOTO_MIN_BIAS_COUNT=225`이며, 설치 APK의 `stt_bias_count`가 이 값보다 낮으면 `stale_or_missing_bias`로 실패시켜 generated full/polite/clipped photo bias hint가 빠진 구버전 APK 측정을 막는다. 접근성 서비스가 Android 설정에는 남아 있지만 bind되지 않았거나 `Crashed services`에 들어간 경우는 `accessibility_not_bound` 또는 `accessibility_crashed`로 분류한다. HyperOS dumpsys는 `Bound services`에 component 대신 `Service[label=Jarvis]`만 표시할 수 있으므로, 이 포맷도 bound 상태로 인정한다. 자동 smoke 검증이 필요할 때는 `JARVIS_PHOTO_LIVE_INJECT_COMMAND=take_photo`를 지정해 실제 발화 없이 같은 실행 경로를 점검한다.
