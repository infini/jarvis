...
- 2026-06-21 실기기 trace에서 owner gate 통과 발화와 command window 입력 peak RMS가 `0.005`대인 케이스가 확인되어, live local ASR 말소리 판정 기준을 `0.0035` RMS로 낮췄다. 이후 `VOICE_RECOGNITION` source 적용 후에도 낮은 입력이 반복되어 command fallback 기준은 `0.0020`, activation 기준은 `0.0012` RMS로 낮췄다.
