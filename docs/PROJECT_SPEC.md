...
- 2026-06-23 최신 debug APK를 Xiaomi 15 Ultra에 재설치한 뒤 촬영 없이 1초 debug command window를 열어 실제 기기 로그에서 `biasCount=159`, `minMs=180`, `possibleSilenceMs=90`, `completeSilenceMs=180`, `listen_start=60ms`, `ready_for_speech=90ms`를 확인했다. 이전 설치본 로그의 `biasCount=109`는 구버전 APK였으므로, 이후 `자비스 사진 찍어` 인식률 평가는 `scripts/jarvis-photo-live-check.sh`의 `expected_min=159` stale guard를 통과한 APK로만 판단한다.
...
- `partial_results`: partial STT 결과 수신. Android STT 후보를 최대 5개까지 `candidates=a|b|c` 형태로 기록한다.
