...
- 2026-06-23 최신 debug APK를 Xiaomi 15 Ultra에 재설치한 뒤 촬영 없이 1초 debug command window를 열어 실제 기기 로그에서 `biasCount=225`, `maxResults=8`, `minMs=180`, `possibleSilenceMs=90`, `completeSilenceMs=180`, `listen_start=50ms`, `ready_for_speech=75ms`를 확인했다. 이후 붙여쓰기 사진 명령 hint를 추가한 설치본에서 `biasCount=267`, `maxResults=8`, `minMs=180`, `possibleSilenceMs=90`, `completeSilenceMs=180`, `listen_start=49ms`, `ready_for_speech=70ms`를 확인했다. command hint 목록과 STT timing 객체는 반복 listen마다 재생성하지 않고 앱 프로세스 안에서 재사용하며, 재사용 변경 후 설치본에서도 `biasCount=267`, `listen_start=51ms`, `ready_for_speech=80ms`를 확인했다. 명령 파서는 STT 후보별 정규화를 한 번만 수행하고 정규식/단어 목록을 재사용하며, 이 변경 후 설치본에서는 `biasCount=267`, `listen_start=51ms`, `ready_for_speech=65ms`를 확인했다. 이후 suffix ASR variant 패턴도 미리 계산해 후보별 파싱 중 중복 조합 생성을 없앴고, 2026-06-23 재설치 주입 live check에서 `stt_bias actual=271`, `android_ready=27ms`, `access=18ms`, `shutter=19ms`, `access_shutter=1ms`, `shutter_result=coordinate`를 확인했다. 사진 문맥의 촬영 동사 ASR 변형은 suffix 기반으로 판정하며 `찌겨`, `지겨`, `치겨`, `찍혀`를 추가로 허용하되 `사진 지겨워`, `사진 지켜봐` 같은 비명령 suffix는 촬영이나 `켜` 기반 카메라 실행으로 처리하지 않는다. `자비스 사진 찍어` 인식률 평가는 `scripts/jarvis-photo-live-check.sh`의 `expected_min=271` stale guard를 통과한 APK로만 판단한다. 이전 설치본 로그의 `biasCount=109`, 225, 267은 구버전 APK로 본다.
