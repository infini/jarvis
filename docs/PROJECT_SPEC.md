...
`scripts/jarvis-photo-live-check.sh`는 카메라 앱을 연 뒤 debug command window를 시작하고 사용자가 `자비스 사진 찍어`를 한 번 말하는 실발화 검증용 스크립트다. `partial_results`, `final_results`, `parse_no_command`의 후보 목록, `command_parsed command=take_photo`, `parsed_candidate_index`, `stt_bias_count`, STT endpoint timing, `accessibility_command_received`, 셔터 좌표 fast path 로그를 함께 확인한다. 접근성 서비스가 Android 설정에는 남아 있지만 bind되지 않았거나 `Crashed services`에 들어간 경우는 `accessibility_not_bound` 또는 `accessibility_crashed`로 분류한다. 자동 smoke 검증이 필요할 때는 `JARVIS_PHOTO_LIVE_INJECT_COMMAND=take_photo`를 지정해 실제 발화 없이 같은 실행 경로를 점검한다.
