...
- Android STT command window는 짧은 명령 구문에 맞춰 `LANGUAGE_MODEL_WEB_SEARCH`, minimum input 180ms, possibly-complete silence 90ms, complete silence 180ms를 사용한다. command window에 한해 `CommandCatalog`의 지원 문구와 빠른 partial prefix, clipped photo prefix, 사진 촬영 동사 오인식 문구, 대표 호출어 오인식 문구, 호출어 오인식과 완성형/공손형/partial/clipped/ASR variant 촬영 동사를 조합한 문구를 `EXTRA_BIASING_STRINGS`로 전달해 Android STT가 지원할 경우 `자비스 사진 찍어` 같은 짧은 명령 후보를 우선 올리도록 유도한다. 2026-06-23에는 Android STT가 중간 단어 `사진`을 누락해 `자비스 찍`, `자베스 찍`, `쟈비스 찌`, `제이비스 찍`, `서비스 찌`처럼 반환하는 경우도 final 대기 없이 `take_photo` partial 후보로 인정하고, partial callback 없이 final-only로 같은 exact short 후보 또는 `자비스 사진 지` 같은 clipped 후보가 들어와도 후보 순서대로 strict final parse 후 `final_fast_partial` fallback으로 처리하도록 확장했다. 이후 호출어 오인식별 `자베스 사진 찍어`, `쟈비스 사진 찍어`, `자 비서 사진 찍어줘`, `서비스 사진 찍어 줘`, `자베스 사진 찍어주세요`, `서비스 사진 찍어 주세요` 같은 완성형/공손형 촬영 후보도 자동 생성해 실제 사용자가 말하는 문장이 STT 후보에 더 잘 오르도록 했다. 추가로 한국어 STT가 `사진 찍어`를 `사진찍어`처럼 붙여 반환하거나 붙은 형태를 더 잘 끌어올릴 수 있도록 대표 호출어에 한해 `자비스 사진찍어`, `자베스 사진찍어줘`, `쟈비스 사진찍어주세요`, `제이비스 사진찍`, `자비서 사진찌`, `서비스 사진치` 같은 붙여쓰기 사진 명령 hint를 보강했다. 호출어가 `자비서`, `제이비스`, `서비스`로 흔들리면서 촬영 동사도 `지거`, `치겨`, `찍혀`처럼 같이 흔들리는 후보도 bias에 추가한다. 빠른 실행은 final보다 partial command path로 우선 달성한다.
- command window Android STT는 partial fast path 속도를 유지하면서 final-only 복구율을 높이기 위해 `EXTRA_MAX_RESULTS=8`을 요청한다. `SpeechCommandSelector`는 Android가 반환한 전체 후보 순서를 보존해 strict parse 후 후보별 `final_fast_partial` fallback을 적용하고, 로그와 photo diagnostic도 최대 8개 후보를 남긴다. idle wake 쪽은 기존처럼 최대 5개 후보를 유지한다.
...
JarvisVoiceService
  ...
  ├─ SpeechCommandSelector: final/partial 후보에서 실행할 command와 source 선택
...
| `SpeechCommandSelector.kt` | final/partial STT 후보 순서를 보존하면서 strict parse, `final_fast_partial`, `partial` 경로를 선택하고 후보 순번/source를 기록 |
...
사진 촬영 명령은 `자비스 사진 찍어`, `자비스 사진 찍어줘`, `자비스 사진 찍어 주세요`, `자비스 찍어`, `자비스 셔터`, `자비스 셔터 눌러`, `자비스 촬영해줘`, `자비스 찰칵`을 `take_photo`로 매핑한다. `자비스 사진 찍어`의 partial 인식 속도를 높이기 위해 문장이 `자비스사진찍` 또는 `자비스사진찌` 패턴으로 끝나는 경우도 촬영 명령으로 인정한다. 더 빠른 partial command path에서는 `자비스사진지`, `자비스사진치`로 끝나는 중간 후보도 `take_photo`로 인정한다. final 결과에서는 STT 후보 순서대로 각 후보에 strict parser를 먼저 적용하고, 실패한 후보에 한해서 `final_fast_partial` fallback을 적용해 partial callback 없이 들어온 `자비스사진지`, `자비스사진치` clipped 후보를 복구한다. Android STT가 사진 문맥에서 `찍어`를 `찌거`, `찌꺼`, `지거`, `지꺼`, `치거`, `치꺼`, `찌겨`, `지겨`, `치겨`, `찍혀`, `지켜`, `치켜`처럼 반환하는 경우도 촬영 동사 ASR 변형으로 인정한다. 이 변형은 suffix 기반으로만 인정해 `자비스 사진 지겨워`, `자비스 사진 지켜봐`처럼 뒤에 다른 단어가 이어지는 문장은 실행하지 않는다. 단, `자비스 지켜`처럼 사진 문맥이 없는 오인식 후보, `자비스 사진`처럼 촬영 동사가 없는 문장, `자비스 사진 찍지 마`처럼 뒤에 다른 동사가 이어지는 문장은 실행하지 않는다.
...
live command는 Android 기본 `SpeechRecognizer`가 먼저 듣고 partial/final 결과에서 명령을 파싱한다. Android STT command window는 `LANGUAGE_MODEL_WEB_SEARCH`, minimum input 180ms, possibly-complete silence 90ms, complete silence 180ms를 사용하고, `CommandCatalog`의 전체 지원 문구와 hidden recognition hints를 `EXTRA_BIASING_STRINGS`로 함께 전달한다. hidden hints는 command wake 후보별로 `사진 찍어`, `사진 찍어줘`, `사진 찍어 줘`, `사진 찍어주세요`, `사진 찍어 주세요`, `사진 찍`, `사진 찌`, `사진 지`, `사진 치`, direct `찍`, direct `찌` 조합을 자동 생성하고, 대표 호출어에는 `사진찍어`, `사진찍어줘`, `사진찍어주세요`, `사진찍`, `사진찌`, `사진지`, `사진치` 같은 붙여쓰기 사진 명령 조합도 추가한다. `자비스 사진 치거`, `자비서 사진 지거`, `제이비스 사진 치겨`, `서비스 사진 찍혀`, `자비스 찌거`, `서비스 지꺼` 같은 촬영 동사 오인식 후보도 함께 추가한다. 이 biasing extra는 recognizer 구현체가 지원할 때만 효과가 있으며, idle wake/activation intent에는 넣지 않는다. command window를 열면 추가 대기 없이 STT를 시작하고, debug command window도 동일하게 즉시 시작한다. Android STT가 발화 시작이나 partial을 감지한 뒤 실패했거나 사용할 수 없을 때만 local ASR fallback을 사용하며, fallback은 `0.0012` RMS 이상의 160ms active speech, 최소 560ms 청취, 240ms trailing silence가 감지되면 6초 live listen timeout 전에도 final decode를 실행한다. Android STT가 발화 시작/partial 없이 `NO_MATCH` 또는 timeout을 반환하면 실패음/빨간 표시 없이 command window 안에서 조용히 다시 듣고, command window deadline은 연장하지 않는다. local ASR 입력은 원본 RMS가 `0.00008` 이상일 때 목표 RMS `0.04`, 최대 `30x`까지 gain을 적용한다. 실제 사용자 준비음/진동은 Android STT `ready_for_speech` callback에서 낸다. 빠른 partial 명령은 final 결과를 기다리지 않고 즉시 실행한다. `parseFastPartial()`은 `자비스사진찍어`, `자비스사진찍어줘`, `자비스사진지`, `자비스사진치`, 사진 문맥의 촬영 동사 ASR variant suffix, direct `자비스찍`/`자비스찌`를 전체 parser보다 먼저 검사해 사진 partial 후보를 짧은 경로로 끝낸다. final 결과에서는 `SpeechCommandSelector`가 STT 후보 순서대로 strict parser를 먼저 적용하고, 실패한 후보에 한해서 `parseFastPartial()`을 `final_fast_partial` source로 다시 적용해 partial callback이 누락된 clipped 후보를 복구한다. partial/final fast path는 final parser보다 짧은 `자비스사진지`, `자비스사진치` 후보를 추가로 인정한다. exact direct short 후보인 `자비스찍`, `자비스찌`, `자베스찍`, `쟈비스찌`, `제이비스찍`, `서비스찌`는 partial과 final-only 결과 모두에서 `take_photo`로 인정하되, 호출어/사진 문맥이 없거나 `자비스지금찍`처럼 뒤에 다른 단어가 끼는 후보는 실행하지 않는다. cancel callback이 오지 않는 경우에도 20ms 뒤 recognizer를 재생성하고 다음 흐름으로 넘어간다. 명령 처리 후 연속 command STT는 확인음 없이 즉시 시작한다. deadline 이후 Android STT가 speech-active 상태로 결과를 붙잡고 있으면 3.5초 grace 뒤 취소하며, listening timeout은 발화 중인 recognizer를 먼저 취소하지 않는다. `home`, `back`은 현재 앱만 제어하고 command window를 유지한다. `stop_listening`, `stop_service`, `wake_screen`, `sleep_screen`도 partial command path에서 빠르게 실행할 수 있다. `자비스 잠들어`와 `stop_listening`은 command window를 닫고 service를 종료한다. `자비스 완전 종료`, `자비스 서비스 종료`, notification `Jarvis 종료`도 service를 종료한다.

`JarvisCommandExecutor`는 중복 명령을 막기 위해 기본 1400ms cooldown을 둔다. `take_photo`는 빠른 반복 촬영 UX를 위해 500ms cooldown을 사용한다. partial/final 중복 실행은 `partialCommandHandled`와 이 짧은 cooldown이 함께 막는다.
