...
- 2026-06-23 앱 메인 화면에 `명령어 리스트` 항목을 유지하고, 목록에서는 전체 지원 명령의 대표 문구를 보여주며, 명령 선택 시 전체 인식 문구, 실행 동작, 상세 설명, 필요 조건, 명령 후 상태, 잘린 final 후보 복구 정책을 확인할 수 있게 했다.
- 2026-06-25 `명령어 리스트` 상세 화면에서 명령어별 음성 샘플을 3초 녹음하고 삭제할 수 있게 했다. 샘플은 앱 private storage의 `command_voice_samples/<commandId>/` 아래 WAV/JSON 쌍으로 저장하고 명령어당 최신 12개만 유지한다. 2026-07-18부터 `CommandVoiceSampleMatcher`는 파일 수가 아니라 실제 feature를 만든 서로 다른 유효 exemplar 2개가 모두 거리/길이 기준을 통과하고 다른 유효 consensus 명령과의 margin도 확보된 경우에만 `voice_sample_match` endpoint를 반환한다. debug APK는 local command no-command와 `voice_sample_match` 발화를 cache `command-recognition-attempts/`에 WAV/JSON으로 저장하고, `jarvis-command-replay.sh`가 저장 캡처와 명령어별 샘플을 replay해 matcher distance/reason을 로그로 남긴다. 목록에는 명령어별 저장 샘플 수와 최근 녹음 정보가 표시된다.
- 2026-06-26 품질 정리로 repo-local Gradle wrapper와 `scripts/jarvis-quality-check.sh`를 추가했다. 2026-07-18에는 상태와 접근성 명령 전달을 모두 프로세스 내부 listener로 전환해 API 26~32의 외부 broadcast 주입 경로를 제거했다. Android 13 이상에서만 `EXTRA_BIASING_STRINGS`를 전달하고, Android 12+ data extraction/full backup rules는 앱 private 음성 샘플과 owner profile이 백업/전송되지 않도록 명시적으로 제외한다.
- 2026-07-18 음성 경로 리뷰에서 partial 실행 제거와 final-only 확정, 전체 final 부정 veto, 신뢰도별 호출어 alias 분리, final-result watchdog, local/Android recognizer generation, 2-exemplar sample consensus, command-only ASR warm-up을 반영했다. 세부 내용은 [`CODE_REVIEW_2026-07-18.md`](CODE_REVIEW_2026-07-18.md)를 기준으로 한다.
...
- 접근성 서비스 watchdog이 `JarvisVoiceService`를 15초마다 되살리던 동작은 제거했다. 접근성 서비스는 이제 카메라/시스템 조작 명령을 실행하는 역할만 맡고, 음성 세션 시작은 기본 어시스턴트 호출, boot notification 탭, 앱의 `Jarvis에게 말하기` 버튼이 담당한다.
...
- 완전 종료는 `자비스 완전 종료`, `자비스 서비스 종료`, foreground notification의 `Jarvis 종료` 액션으로 처리한다. `자비스 잠들어`/`멈춰`는 현재 명령 세션을 닫는 가벼운 종료이고, 전원 버튼 또는 앱 버튼으로 다시 호출할 수 있다.
- 아래 2026-06-20~2026-06-21 wake/owner-gate/idle guard 기록은 상시 음성 대기 구현과 디버깅 이력이다. 호출형 정책과 충돌하는 경우 2026-06-22 호출형 정책을 우선한다.
...
- `멈춰`는 현재 command window를 닫는 명령으로 유지한다. 호출형 정책에서는 command window가 닫히면 음성 서비스도 내려가므로, 이후 재호출은 기본 어시스턴트 호출이나 앱 버튼으로 수행한다.
- Jarvis 서비스는 더 이상 재부팅 전까지 foreground service로 유지하지 않는다. 실행 중인 command window가 있을 때만 foreground notification과 마이크를 사용한다.
- 접근성 서비스가 `JarvisVoiceService` foreground service를 자동 재시작하던 watchdog은 배터리 정책상 제거했다.
- Jarvis 서비스 실행 중에는 마이크 점유 충돌을 피하기 위해 소유자 목소리 재등록 시작 시 음성 서비스를 잠시 중지한다. 등록이 성공해도 음성 서비스를 자동 재시작하지 않고, 다음 기본 어시스턴트 호출 또는 `Jarvis에게 말하기` 버튼으로 시작한다.
- command window의 watchdog timeout을 12초로 늘려 명령 대기 중 불필요한 STT 재시작을 줄였다. partial 명령 실행 후 다음 리스닝 전환 대기는 초기 80ms로 줄였고, 현재는 command handled 확인음 제거 후 0ms로 줄였다.
...
- live command는 Android 기본 STT가 먼저 듣는다. partial은 1순위 후보와 사진 명령 진단을 기록할 뿐 부작용을 실행하지 않고, 모든 명령은 final에서만 확정한다. Android STT가 실제 발화를 감지한 뒤 실패했을 때만 local ASR fallback이 6초 동안 첫 명령 발화를 기다리고, 아무 말이 없거나 텍스트 없는 360ms 미만의 짧은 소리만 있으면 local 대기를 이어간다. Android STT가 발화 시작/partial 없이 `NO_MATCH` 또는 timeout을 반환하면 실패 피드백 없이 command window 안에서 조용히 다시 듣는다. 발화 종료 후 2.5초 안에 final/error가 없으면 recognizer를 재생성하고, instance generation으로 폐기된 recognizer의 지연 callback을 버린 뒤 command window 안에서 다시 듣는다.
- Android STT command window는 짧은 명령 구문에 맞춰 `LANGUAGE_MODEL_WEB_SEARCH`, minimum input 180ms, possibly-complete silence 90ms, complete silence 180ms를 사용한다. command window에 한해 `CommandCatalog`의 지원 문구와 clipped photo prefix, 사진 촬영 동사 오인식 문구, 대표 호출어 오인식 문구, 호출어 오인식과 완성형/공손형/partial/clipped/ASR variant 촬영 동사를 조합한 문구를 `EXTRA_BIASING_STRINGS`로 전달해 Android STT가 지원할 경우 `자비스 사진 찍어` 같은 짧은 명령 후보를 우선 올리도록 유도한다. 2026-06-23에는 Android STT가 중간 단어 `사진`을 누락해 `자비스 찍`, `자베스 찍`, `쟈비스 찌`, `제이비스 찍`, `서비스 찌`처럼 반환하는 경우도 `take_photo` strict final 후보로 인정하고, partial callback 없이 final-only로 같은 exact short 후보는 strict parse하고, `자비스 사진 지` 같은 clipped 후보는 strict 후보가 전부 실패한 뒤 `final_fast_partial` fallback으로 처리하도록 확장했다. 이후 호출어 오인식별 `자베스 사진 찍어`, `쟈비스 사진 찍어`, `자 비서 사진 찍어줘`, `서비스 사진 찍어 줘`, `자베스 사진 찍어주세요`, `서비스 사진 찍어 주세요` 같은 완성형/공손형 촬영 후보도 자동 생성해 실제 사용자가 말하는 문장이 STT 후보에 더 잘 오르도록 했다. 추가로 한국어 STT가 `사진 찍어`를 `사진찍어`처럼 붙여 반환하거나 붙은 형태를 더 잘 끌어올릴 수 있도록 대표 호출어에 한해 `자비스 사진찍어`, `자베스 사진찍어줘`, `쟈비스 사진찍어주세요`, `제이비스 사진찍`, `자비서 사진찌`, `서비스 사진치` 같은 붙여쓰기 사진 명령 hint를 보강했다. 호출어가 `자비서`, `제이비스`, `서비스`로 흔들리면서 촬영 동사도 `지거`, `치겨`, `찍혀`처럼 같이 흔들리는 후보도 bias에 추가한다. 2026-07-18부터 partial은 진단만 남기고 실행은 final에서 확정한다.
- command window Android STT는 final-only 복구율과 후보 다양성을 높이기 위해 `EXTRA_MAX_RESULTS=8`을 요청한다. `SpeechCommandSelector`는 어느 후보든 명시적인 부정/취소가 있으면 전체를 거절하고, 그렇지 않으면 모든 후보의 strict parse를 순서대로 검사한 뒤 전부 실패한 경우에만 `final_fast_partial` fallback을 적용한다. 로그와 photo diagnostic은 최대 8개 후보를 남기고, idle wake 쪽은 기존처럼 최대 5개 후보를 유지한다.
- 2026-06-23 최신 debug APK를 Xiaomi 15 Ultra에 재설치한 뒤 촬영 없이 1초 debug command window를 열어 실제 기기 로그에서 `biasCount=225`, `maxResults=8`, `minMs=180`, `possibleSilenceMs=90`, `completeSilenceMs=180`, `listen_start=50ms`, `ready_for_speech=75ms`를 확인했다. 이후 붙여쓰기 사진 명령 hint를 추가한 설치본에서 `biasCount=267`, `maxResults=8`, `minMs=180`, `possibleSilenceMs=90`, `completeSilenceMs=180`, `listen_start=49ms`, `ready_for_speech=70ms`를 확인했다. command hint 목록과 STT timing 객체는 반복 listen마다 재생성하지 않고 앱 프로세스 안에서 재사용하며, 재사용 변경 후 설치본에서도 `biasCount=267`, `listen_start=51ms`, `ready_for_speech=80ms`를 확인했다. 명령 파서는 STT 후보별 정규화를 한 번만 수행하고 정규식/단어 목록을 재사용하며, 이 변경 후 설치본에서는 `biasCount=267`, `listen_start=51ms`, `ready_for_speech=65ms`를 확인했다. 이후 suffix ASR variant 패턴도 미리 계산해 후보별 파싱 중 중복 조합 생성을 없앴고, partial 사진 후보는 전체 파서보다 먼저 사진 전용 suffix fast path에서 처리한다. 호출어와 촬영 동사가 동시에 흔들리는 후보를 bias에 추가한 설치본은 `stt_bias actual=325`, `android_ready=38ms`, `access=23ms`, `shutter=24ms`, `access_shutter=1ms`, `shutter_result=coordinate`를 확인했다. 사진 문맥의 촬영 동사 ASR 변형은 suffix 기반으로 판정하며 `찌겨`, `지겨`, `치겨`, `찍혀`를 추가로 허용하되 `사진 지겨워`, `사진 지켜봐` 같은 비명령 suffix는 촬영이나 `켜` 기반 카메라 실행으로 처리하지 않는다. `자비스 사진 찍어` 인식률 평가는 `scripts/jarvis-photo-live-check.sh`의 `expected_min=325` stale guard를 통과한 APK로만 판단한다. 이전 설치본 로그의 `biasCount=109`, 225, 267, 271은 구버전 APK로 본다.
...
- 저장된 소유자 목소리 embedding이 2개 미만이거나 `자비스 깨어나` activation 문구로 등록되지 않은 구버전 프로필은 `profile_configured=false`로 취급하고, Jarvis command window 시작을 차단한다. 재등록 성공 후에도 음성 서비스를 자동 상시 대기로 시작하지 않는다.
- 속도 측정 리포트는 activation ASR, owner verification, Android STT, 접근성 실행 구간을 분리해 출력한다.
- `CommandBus`는 연결된 접근성 서비스의 프로세스 내부 receiver로만 명령을 전달한다. receiver가 없으면 전송이 거절되고, 카메라 접근성 제스처는 완료/취소 callback, 동기 시스템 동작은 dispatch 접수 결과로 성공·실패 상태를 전환한다. 2026-06-23 주입 live check 기준 `take_photo`는 `access=4ms`, `shutter=5ms`, `access_shutter=1ms`, `shutter_result=coordinate`로 확인됐다.
...
8. `app/src/main/java/com/personal/jarvis/JarvisFeedbackController.kt`: 상태별 소리/진동/프로세스 내부 상태 전달 정책 확인
...
- 전원 버튼 길게 누르기 또는 앱의 `Jarvis에게 말하기`: Jarvis command window 30초 열기
- `자비스 카메라 실행`: 기본 카메라 앱 실행
- `자비스 카메라 전면`: 기본 카메라 앱을 전면 카메라 힌트와 함께 실행 또는 전면으로 전환
- `자비스 카메라 후면`: 기본 카메라 앱을 후면 카메라 힌트와 함께 실행 또는 후면으로 전환
- `자비스 사진 찍어`: 현재 카메라 화면에서 셔터 버튼 탭
- `자비스 카메라 종료`: 카메라 앱에서 홈으로 이동
- `자비스 화면 켜`: 꺼진 화면을 깨워 잠금화면 표시
- `자비스 화면 꺼`: 접근성 잠금화면 전역 액션으로 화면 끄기/잠금
- `자비스 뒤로`: Android 뒤로가기
- `자비스 홈`: Android 홈으로 이동
- `자비스 잠들어` 또는 `자비스 멈춰`: 현재 command window 닫기
- `자비스 완전 종료` 또는 notification `Jarvis 종료`: Jarvis 음성 foreground service 종료
...
- Assistant Activity: Android `ACTION_ASSIST`/`ACTION_VOICE_COMMAND` 시스템 호출을 받아 Jarvis command window 시작
- Foreground Service: command window가 열린 동안만 마이크 기반 음성 인식 실행
- LocalCommandRecognizer: Android STT 실패 fallback을 sherpa-onnx streaming ASR로 인식. 2026-06-22 호출형 정책에서는 idle activation loop를 기본 시작하지 않는다.
...
일반 Android 앱은 전원 버튼 long press 이벤트를 직접 가로챌 수 없다. Jarvis가 Gemini 대신 호출되려면 사용자가 Android/HyperOS 설정에서 Jarvis를 기본 디지털 어시스턴트 앱으로 선택해야 한다. 전원 버튼 long press가 기본 어시스턴트를 실행할지, 전원 메뉴를 열지, Gemini를 강제할지는 OS/OEM 설정에 달려 있다. Jarvis 앱은 이 설정을 자동으로 바꾸지 않고, `설정 관리 → 전원 버튼으로 빠르게 호출`에서 시스템 RoleManager 요청 화면만 연다.

`JarvisAssistantActivity`는 시스템 signature 권한인 `android.permission.BIND_VOICE_INTERACTION`을 요구해 일반 앱의 explicit Activity 호출을 차단한다. 시스템 어시스턴트 호출을 받은 뒤 command window 시작 전에 마이크 권한, Jarvis 접근성 서비스 설정값, 현재 프로세스의 접근성 direct receiver 연결, 소유자 프로필 설정을 확인한다. 접근성 서비스가 꺼져 있거나 Android에 bind되지 않은 상태면 하이퍼아일랜드 overlay와 카메라 세부 제어가 불가능하므로 command window를 열지 않고 `MainActivity`로 이동해 설정을 안내한다.

전면/후면 카메라 실행은 Android 카메라 인텐트에 렌즈 방향 힌트 extra를 넣은 뒤, 접근성 서비스에서 Xiaomi 카메라의 `com.android.camera:id/v9_camera_picker` 노드를 읽어 현재 렌즈를 확인한다. content description은 실기기에서 `전후면 카메라 전환,후면` 또는 `전후면 카메라 전환,전면` 형태로 노출된다. 다른 카메라 앱의 `Switch to front camera`처럼 클릭 시 전환 대상을 설명하는 라벨은 현재 렌즈의 반대로 해석하며, 의미가 모호한 라벨은 추측하지 않는다. 현재 렌즈가 목표와 다를 때만 전환 버튼을 클릭하고 500ms 간격으로 다시 확인한 뒤 성공을 알린다.

Android 14(API 34)+에서 `RECORD_AUDIO`는 while-in-use 권한으로 취급된다. 현재 앱은 `targetSdk=35`이므로 `BOOT_COMPLETED` receiver에서 microphone foreground service를 직접 시작할 수 없다. 배터리 정책상 Jarvis는 부팅 후 마이크 대기를 자동 시작하지 않는다.

부팅/앱 업데이트 후 동작:

1. `JarvisBootReceiver`가 `BOOT_COMPLETED` 또는 `MY_PACKAGE_REPLACED`를 받는다.
2. receiver는 마이크 서비스를 직접 시작하지 않고 `Jarvis 준비됨` 알림을 띄운다.
3. 사용자가 알림을 누르면 `JarvisAssistantActivity`가 열리고 30초 command window를 시작한다.
4. command window가 닫히면 `JarvisVoiceService`는 `START_NOT_STICKY`로 종료된다.
5. 접근성 서비스는 음성 서비스를 자동 재시작하지 않는다.

완전한 “부팅 직후 무터치 마이크 대기”는 배터리 소모와 Android 백그라운드 제한 때문에 현재 목표에서 제외한다. 장기적으로 상시 대기가 다시 필요해지면 VoiceInteractionService 기반의 기본 Assistant 구현, device-owner/system app, 또는 사용자가 명시적으로 유지한 foreground session 중 하나를 별도 검토한다.

## 7. Current Architecture

```text
전원 버튼 long press / 기본 어시스턴트 호출 / 앱 버튼
  ↓
JarvisAssistantActivity 또는 MainActivity
  ↓
JarvisVoiceService
  ├─ JarvisVoiceServiceStarter: command window startForegroundService helper
  ├─ LocalActivationSession → LocalCommandRecognizer: legacy/debug idle activation phrase streaming ASR
  ├─ OwnerVoiceEngine: 소유자 목소리 등록/프로필 검증
  ├─ OwnerVoiceGate → OwnerVoiceEngine: legacy/debug owner gate
  ...
  ├─ SpeechCommandSelector: final 실행 command 선택과 partial 1순위 진단
  ├─ JarvisNotificationController: foreground notification 상태 표시
  ├─ JarvisFeedbackController → JarvisStateBus: 소리/진동/프로세스 내부 상태 listener
...
| File | Role |
| --- | --- |
| `MainActivity.kt` | 준비 상태 hero, 단일 `Jarvis에게 말하기` CTA, 실시간 음성 상태, 대표 명령과 설정/지원 명령 진입 UI |
| `SetupActivity.kt` | 마이크, 소유자 목소리, 접근성 필수 온보딩과 기본 어시스턴트/알림/배터리 권장 설정 UI |
| `JarvisSetupStatus.kt` | 필수/권장 준비 상태 계산과 다음 필수 단계 선택 |
| `JarvisUi.kt` | 사용자/debug 화면의 공통 색상, 카드, 버튼, system bar/cutout safe inset 처리 |
...
| `CommandListActivity.kt` | `지원 명령` 화면. 대표 문구, 사용자 중심 동작/필요 조건과 `내 발음으로 인식 개선` 진입을 표시하고 내부 STT 정책/command id는 노출하지 않음 |
| `CommandVoiceSamplePanel.kt` | `지원 명령` 상세 화면의 명령어별 샘플 수, 녹음, 삭제, 권한 요청 후 녹음 재개 UI 처리 |
...
| `CommandVoiceSampleRecorder.kt` | `지원 명령` 상세 화면에서 실행되는 3초 명령어 샘플 녹음 workflow, 진행률, 완료/실패 callback |
...
| `JarvisVoiceServiceStarter.kt` | 앱 UI/어시스턴트 Activity에서 공통으로 쓰는 command window 시작 helper |
...
| `debug/JarvisDeveloperMenuActivity.kt` | debug APK 내부 전용 카메라/셔터/화면 동작 테스트 및 replay/profile 진단 메뉴 |
...
| `SessionGeneration.kt` | 중단된 local worker와 폐기된 Android recognizer의 늦은 callback이 새 세션에 반영되지 않도록 generation token 관리 |
| `SpeechRecognitionIntentFactory.kt` | Android `SpeechRecognizer` intent/timing option 생성 |
| `SpeechCommandSelector.kt` | 전체 final 부정 veto, strict 우선, 이후 `final_fast_partial` fallback과 partial 1순위 진단 |
...
| `JarvisFeedbackController.kt` | Jarvis 상태별 소리, 진동, 프로세스 내부 상태 전달 |
| `JarvisVoiceState.kt` | Jarvis 상태 enum |
| `JarvisStateBus.kt` | 음성 서비스에서 접근성 서비스로 상태 전달 |
| `CommandInterpreter.kt` | 인식된 문장을 내부 명령으로 변환 |
| `CommandBus.kt` | 접근성 서비스로 보내는 프로세스 내부 명령 버스와 dispatch 결과 |
| `JarvisAccessibilityService.kt` | 접근성 서비스 생명주기, 명령 dispatch. 음성 서비스 자동 재시작은 하지 않음 |
...
- command window가 열리기 전에는 overlay를 띄우지 않는다. 호출형 정책에서는 평상시 마이크 대기가 없으므로 화면 위 지속 표시가 없어야 한다.
- legacy/debug activation phrase 확인 중에도 overlay를 띄우지 않는다. command window가 실제로 열린 경우에만 명확히 표시한다.
- notification 문구는 active command window 진단용으로만 유지한다. command window가 닫히면 foreground service도 종료된다.
...
스크립트는 `trace`, `total`, `path`, `command`, `status`, `owner_gate`, `listen`, `listen_ready`, `parsed`, `parsed_source`, `parsed_candidate_index`, `speech_parse`, `access`, `speech_access`, `command_access`, `bus`, `shutter`, `speech_shutter`, `command_shutter`, `access_shutter`, `shutter_result`, `stt_bias_count`, `stt_max_results`, `stt_min_ms`, `stt_possible_silence_ms`, `stt_complete_silence_ms`를 한 줄로 출력한다. 하위 줄에는 `activation_endpoint`, `activation_elapsed`, `activation_text`, `activation_owner`, `local_endpoint`, `local_elapsed`, `local_text`, `android_ready`, `speech_begin`, `speech_end`, `error`, `peak_rms`, `mean_rms`, `asr_gain`을 출력해 local activation ASR, owner voice verification, live Android STT, local fallback 병목을 분리한다. `listen`은 Android STT 시작 시점이고, `listen_ready`는 Android STT 시작부터 `ready_for_speech`까지의 시간이다. `parsed`는 trace 시작부터 명령 파싱까지의 누적 시간이고, `parsed_source`는 `final`, `final_fast_partial`, `local` 같은 파싱 경로이며, `parsed_candidate_index`는 Android STT 후보 중 실제로 채택된 후보 순번이다. `speech_parse`는 Android STT가 발화를 감지한 뒤 명령 파싱까지의 시간이다. `speech_access`는 Android STT 발화 시작부터 접근성 서비스 수신까지, `command_access`는 명령 파싱부터 접근성 서비스 수신까지의 시간이다. `shutter`는 trace 시작부터 셔터 제스처 dispatch까지, `command_shutter`는 명령 파싱부터 셔터 dispatch까지, `access_shutter`는 접근성 서비스 수신부터 셔터 dispatch까지의 시간이며 `shutter_result=coordinate`가 셔터 좌표 fast path 성공이다. `jarvis-command-trace.sh`는 debug APK의 no-display `JarvisDebugStartActivity`를 먼저 호출해 Jarvis 서비스를 재시작하려고 시도한다. release APK에는 이 Activity가 포함되지 않으며, 자동 시작은 `JARVIS_START_DEBUG_ACTIVITY=0`으로 끌 수 있다. 현재 activation 등록 기준을 건너뛰고 강제로 측정하려면 `JARVIS_SKIP_PROFILE_CHECK=1`을 지정한다. 측정 중에는 `자비스 깨어나`를 말한 뒤 `JARVIS LISTENING` overlay 또는 준비음/진동이 나오면 `자비스 카메라 실행` 같은 호출어 포함 명령을 말한다. 이 스크립트는 `status=command_complete`가 하나도 없으면 실패 종료하고 `activation_asr_complete`, `activation_owner_verified`, `owner_audio_activation`, `ready_for_speech`, `speech_begin`, `partial_results`, `command_parsed` 카운터로 어느 단계에서 멈췄는지 출력한다. 같은 경로의 `.diagnostic` 로그에는 `OwnerVoiceGate`, `JarvisVoiceService`, `JarvisLatency` 원본을 함께 저장해 owner voice 점수와 activation 거절 사유를 확인할 수 있다. `jarvis-latency-audit.sh <log>`는 기존 측정 로그와 `.diagnostic` 로그를 함께 읽어 `PASS`, `FAIL_LEGACY_PROFILE`, `FAIL_NO_ACTIVATION`, `FAIL_NO_SPEECH`, `FAIL_NO_COMMAND`, `FAIL_NO_COMMAND_COMPLETE`, `FAIL_SLOW`로 분류한다. `FAIL_LEGACY_PROFILE`은 저장된 owner profile이 현재 activation 등록 기준을 만족하지 않아 activation/command latency 판정에 사용할 수 없다는 뜻이고, `FAIL_NO_ACTIVATION`은 local activation hotwords ASR이 `자비스 깨어나` activation phrase를 확인하지 못했거나 owner verification이 거절했다는 뜻이다. 명령이 실행됐더라도 기본 기준 `speech_begin`이 있는 trace에서는 `speech_parse<=2500ms`, `speech_begin`이 없는 trace에서는 `parsed<=2500ms`, 접근성 실행이 있는 경우 `speech_access<=4000ms`와 `command_access<=1200ms`도 함께 검사한다. `jarvis-photo-live-check.sh`는 사진 명령 전용으로 `JARVIS_PHOTO_MAX_PARSED_MS`, `JARVIS_PHOTO_MAX_SPEECH_PARSE_MS`, `JARVIS_PHOTO_MAX_ACCESS_MS`, `JARVIS_PHOTO_MAX_COMMAND_ACCESS_MS`, `JARVIS_PHOTO_MAX_COMMAND_SHUTTER_MS` 기준을 사용하며, 파싱 이후 실제 셔터 dispatch가 `command_shutter<=900ms`를 넘으면 `slow_command_shutter`로 분류한다. 목표는 activation 후 command window 안의 정상 명령이 `path=local_activation_asr->android_stt`, `path=android_stt`, 또는 Android 실패 후 `path=local_asr`, `status=command_complete`로 끝나고, `speech_parse`, `speech_access`, `command_access`, `command_shutter`가 체감 지연을 설명할 수 있는 낮은 값으로 유지되는 것이다. `fallback_to_local` 또는 `local_asr`가 포함된 live trace는 Android STT가 발화를 감지한 뒤 실패해 local fallback을 탔다는 뜻이므로 별도 튜닝 대상으로 본다. `speech_idle_retry`는 command window 안에서 아무 발화가 감지되지 않아 조용히 다시 듣는 정상 idle retry로 본다.
...
- `listen_start`: Android STT 또는 local ASR 리스닝 시작. command Android STT는 `biasCount`, `maxResults`, `minMs`, `possibleSilenceMs`, `completeSilenceMs`도 함께 기록해 실제 recognition hint, 후보 요청 수, 저지연 타이밍 적용 여부를 확인한다.
...
- `fallback_to_local`: Android STT가 발화를 감지한 뒤 실패해 local ASR fallback으로 전환
...
- `partial_results`: partial STT 결과 수신. Android STT 후보를 최대 8개까지 `candidates=a|b|c` 형태로 기록한다.
...
- `final_results`: final STT 결과 수신. Android STT 후보를 최대 8개까지 `candidates=a|b|c` 형태로 기록한다.
...
- `accessibility_command_received`: 접근성 서비스가 프로세스 내부 `CommandBus` 명령을 수신. `transport=direct`와 `source`를 기록한다.
- `accessibility_command_dispatch_return`: 접근성 서비스 command dispatch 반환
- `shutter_tap_start`: `take_photo`가 셔터 제스처 dispatch를 시작
- `shutter_tap_dispatch`: 셔터 제스처 dispatch 결과. `result=coordinate`는 좌표 fast path, `node`는 접근성 노드 fallback, `failed`는 둘 다 실패한 상태다.
- `command_complete`: Jarvis command window 정책까지 반영한 명령 처리 완료
- `command_dispatch_failed`: 접근성 연결 또는 카메라/화면 조작 dispatch 실패

음성 서비스 내부 이벤트의 `total=...ms`는 trace 시작부터 누적 시간이고 `step=...ms`는 직전 이벤트 이후 시간이다. command Android STT `listen_start`는 bias 수와 endpoint timing을, `local_complete`는 로컬 ASR의 elapsed·발화 길이·무음·RMS·gain을, `shutter_tap_dispatch`는 셔터 제스처의 `coordinate`/`node`/`failed` 결과를 기록한다.

debug APK의 `partial_results`, `final_results`, `parse_no_command`는 후보와 사진 명령 분석 값을 상세히 남긴다. release APK에서는 `JarvisLogSanitizer`가 `text`, `first`, `candidates`, `photo` 중 첫 민감 필드부터 이후 detail 전체를 `redacted`로 치환한다. 이 보수적 정책은 발화 자체에 `secret=...` 같은 key 모양 문자열이 있어도 뒤쪽 원문이 로그에 남지 않게 한다.

상태와 접근성 명령은 모두 프로세스 내부 listener로 전달하며 broadcast fallback은 없다. `CommandBus`는 카메라 제스처 완료/취소와 시스템 동작 dispatch 결과를 callback으로 음성 서비스에 돌려주고, 실패 시 `command_dispatch_failed`와 `COMMAND_FAILED`를 발생시킨다. 2026-06-21 상시 wake 실기기 로그에서 얻은 owner gate/activation 교훈은 아래 legacy 섹션에 보존한다.

### Wake/Sleep Recognition Lessons

이 섹션은 2026-06-21 상시 음성 wake 구현에서 얻은 교훈이다. 2026-06-22 이후 기본 UX는 전원 버튼/기본 어시스턴트 호출형이므로, 아래 wake 관련 기준은 legacy/debug 경로에 대한 참고로만 본다.

`자비스 깨어나`와 `자비스 잠들어` 개선은 단순 threshold 조정 문제가 아니었다. 가장 큰 반복 패턴은 “말해도 안 깨어남”을 고치려고 wake 조건을 넓히면 “말하지 않아도 깨어남”이 생기고, false activation을 막으려고 조건을 다시 좁히면 다시 wake miss가 늘어나는 흐름이었다. 상시 wake를 다시 도입할 때는 이 두 증상을 같은 축의 threshold 문제로 보지 말고, activation phrase 검출, owner voice verification, command window lifecycle을 분리해서 봐야 한다.
...
| command window 종료 후 예약된 STT runnable 방치 | `종료`, `자비스 잠들어`, timeout 뒤에도 `ready_for_speech`가 다시 뜨면서 Jarvis가 계속 듣는 것처럼 보였다. | command window close 시 active recognizer, delayed listen runnable, final-result watchdog을 정리한다. |

현재 호출형 정책의 불변 조건:

- Idle 상태에서는 마이크를 계속 열지 않는다.
- command window는 기본 어시스턴트 호출, boot notification 탭, 앱의 `Jarvis에게 말하기` 버튼으로만 연다.
- command window 안에서도 `자비스` 호출어가 포함된 명령만 실행한다.
- `자비스 잠들어`, `멈춰`, `stop_listening`은 현재 command window를 닫고 service를 종료한다.
- `자비스 완전 종료`, `자비스 서비스 종료`, notification `Jarvis 종료`는 service를 즉시 종료하는 명시적 종료 경로다.
- `자비스 카메라 종료`, `자비스 홈`, `자비스 뒤로` 같은 앱 제어 명령은 현재 앱을 제어하고, 카메라 세션 유지 정책에 따라 새 30초 command window를 연다.
...
1. 먼저 `scripts/jarvis-profile-status.sh`로 `profile_configured=true`, `profile_embeddings>=2`, `profile_phrase_id=jarvis_activation_v3`인지 확인한다. 구버전 profile이면 command window 시작 자체를 판단하지 않는다.
...
| `stop_listening` | 현재 command window 닫기 |
| `stop_service` | Jarvis 음성 foreground service 종료 |
...
command window는 기본 어시스턴트 호출, boot notification 탭, 앱의 `Jarvis에게 말하기` 버튼으로 열린다. `자비스 깨어나` 음성 wake는 2026-06-22 호출형 정책의 기본 진입점이 아니다. 관련 activation ASR/owner gate 코드는 debug와 이전 구현 이력으로 남아 있지만, 일반 실행 경로에서는 idle 마이크 대기를 시작하지 않는다.
...
사진 촬영 명령은 `자비스 사진 찍어`, `자비스 사진 찍어줘`, `자비스 사진 찍어 주세요`, `자비스 찍어`, `자비스 셔터`, `자비스 셔터 눌러`, `자비스 촬영해줘`, `자비스 찰칵`을 `take_photo`로 매핑한다. partial 1순위 후보는 진단 로그만 남기고 실행하지 않는다. final 결과에 명시적인 부정/취소 후보가 하나라도 있으면 전체를 거절하고, 그렇지 않으면 모든 후보의 strict parser를 순서대로 적용한 뒤 모두 실패한 경우에만 `자비스사진지`, `자비스사진치` 같은 clipped 후보를 `final_fast_partial`로 복구한다. Android STT가 사진 문맥에서 `찍어`를 `찌거`, `찌꺼`, `지거`, `지꺼`, `치거`, `치꺼`, `찌겨`, `지겨`, `치겨`, `찍혀`, `지켜`, `치켜`처럼 반환하는 경우도 suffix 기반 촬영 동사 ASR 변형으로 인정한다. `자비스 사진 지겨워`, `자비스 사진 지켜봐`, `자비스 사진 찍어 주지 마요`, `자비스 촬영하지 마`, `자비스 카메라 실행 취소해요`처럼 다른 단어·부정·취소가 이어지는 문장은 실행하지 않는다.

command window 안의 명령 파서는 호출어를 문장 시작에서만 인정한다. `제이비스`, `자비서`, `자비써`, `자비쓰`, `자비수`, `잡이스`는 일반 명령에도 허용하는 신뢰 가능한 ASR equivalent다. `서비스`, `잡스`는 사진/촬영 신호가 함께 있을 때만 허용하는 모호한 alias이므로 `서비스 종료`, `스티브 잡스 카메라 실행`은 명령이 아니지만 `서비스 사진 찍어`, `잡스 찍`은 사진 명령 후보가 될 수 있다. 이 보정은 `isActivationWakeAsrEquivalent()`에는 적용하지 않는다.
...
live command는 Android 기본 `SpeechRecognizer`가 먼저 듣고 partial/final 결과를 기록한다. Android STT command window는 `LANGUAGE_MODEL_WEB_SEARCH`, minimum input 180ms, possibly-complete silence 90ms, complete silence 180ms를 사용하고, `CommandCatalog`의 전체 지원 문구와 hidden recognition hints를 `EXTRA_BIASING_STRINGS`로 전달한다. hidden hints는 command wake 후보별 사진 문구, 붙여쓰기, clipped/ASR variant 조합을 포함하며 idle wake/activation intent에는 넣지 않는다. command window와 debug command window는 추가 대기 없이 STT를 시작한다. partial은 1순위 진단만 남기며 어떤 명령도 실행하지 않는다. 실행은 final-only이고, 명시적 부정/취소 후보가 하나라도 있으면 전체를 거절한다. 그 외에는 모든 후보 strict parse 후 전부 실패한 경우에만 `CommandCatalog.fastPartial`이 허용한 `take_photo`의 clipped final fallback을 적용한다. `open_camera_and_take_photo`, `open_camera`, `home`, `back`, 화면/종료 명령은 완성된 strict final에서만 확정한다. 발화 종료 후 2.5초 동안 final/error가 없으면 `final_result_timeout`을 남기고 recognizer를 재생성하며, instance generation으로 이전 recognizer의 지연 callback을 버린 뒤 command window 안에서 다시 듣는다. Android STT가 발화를 감지한 뒤 실패했거나 사용할 수 없을 때만 local ASR fallback을 사용하며, fallback은 `0.0012` RMS 이상의 160ms active speech, 최소 560ms 청취, 240ms trailing silence 조건으로 6초 timeout 전에도 final decode한다. local ASR 입력은 원본 RMS가 `0.00008` 이상일 때 목표 RMS `0.04`, 최대 `30x` gain을 적용한다. 기본 서비스 warm-up은 command recognizer만 초기화하고 legacy activation recognizer는 실제 debug/activation 요청까지 lazy load한다. 명령 처리 후 연속 command STT는 확인음 없이 즉시 시작한다. deadline 이후 speech/final 대기 상태는 3.5초 grace 뒤 정리한다.

`JarvisCommandExecutor`는 중복 명령을 막기 위해 기본 1400ms cooldown을 둔다. `take_photo`는 빠른 반복 촬영 UX를 위해 500ms cooldown을 사용한다. command partial은 실행하지 않으며, final/local 결과가 짧은 시간 안에 중복 전달된 경우 cooldown이 같은 명령의 재실행을 막는다.

`scripts/jarvis-photo-command-audit.sh`는 debug command window에 `take_photo`를 주입해 접근성 서비스 수신, `shutter_tap_dispatch result=coordinate`, command window 재개를 한 번에 검증한다. 이 스크립트는 음성 인식 품질 자체를 증명하지는 않지만, 파서 이후 촬영 실행 경로 지연과 접근성 dispatch 회귀를 빠르게 잡는 기준으로 사용한다.

`scripts/jarvis-photo-live-check.sh`는 카메라 앱을 연 뒤 debug command window를 시작하고 사용자가 `자비스 사진 찍어`를 한 번 말하는 실발화 검증용 스크립트다. `partial_results`, `partial_no_command`, `final_results`, `parse_no_command`의 후보 목록과 사진 명령 진단, `command_parsed command=take_photo`, `parsed_candidate_index`, `stt_bias_count`, STT endpoint timing, `accessibility_command_received`, `shutter_tap_dispatch result=coordinate`를 함께 확인한다. `photo=` 진단에서 `missing_wake`는 호출어 오인식, `missing_shot`은 촬영 동사 누락, `missing_photo_or_direct_shot`은 사진 문맥과 direct 촬영 동사가 모두 부족한 후보로 보고 다음 bias/final 복구 rule 후보를 정한다. 현재 debug 기준은 `JARVIS_PHOTO_MIN_BIAS_COUNT=325`이며, 설치 APK의 `stt_bias_count`가 이 값보다 낮으면 `stale_or_missing_bias`로 실패시켜 generated full/polite/clipped/joined photo bias hint와 suffix ASR variant hint가 빠진 구버전 APK 측정을 막는다. 접근성 서비스가 Android 설정에는 남아 있지만 bind되지 않았거나 `Crashed services`에 들어간 경우는 `accessibility_not_bound` 또는 `accessibility_crashed`로 분류한다. HyperOS dumpsys는 `Bound services`에 component 대신 `Service[label=Jarvis]`만 표시할 수 있으므로, 이 포맷도 bound 상태로 인정한다. 자동 smoke 검증이 필요할 때는 `JARVIS_PHOTO_LIVE_INJECT_COMMAND=take_photo`를 지정해 실제 발화 없이 같은 실행 경로를 점검한다.

`scripts/jarvis-photo-live-series.sh`는 `jarvis-photo-live-check.sh`를 여러 번 반복 실행해 `자비스 사진 찍어`의 체감 인식률과 속도를 수치화한다. 기본은 5회, trial당 12초이며, 각 trial의 원본 출력/latency 로그, 마지막 STT 후보 텍스트, `parsed_candidate_index`, `stt_bias_count`, STT endpoint timing, `parse_no_command` 발생 수, 실패 유형, `parsed_ms`/`speech_parse_ms`/`access_ms`/`speech_access_ms`/`command_access_ms`, `shutter_ms`/`speech_shutter_ms`/`command_shutter_ms`/`access_shutter_ms`, `shutter_result`, 최종 성공률 summary와 TSV 결과 파일을 `/tmp`에 남긴다. summary에는 `final`, `final_fast_partial`, `local` 같은 `parsed_source`, `parsed_candidate_index`, 반복 출현한 `stt_candidate` 빈도를 함께 집계한다. 실패 유형은 `no_ready`, `no_speech`, `no_take_photo_parse`, `wrong_command`, `stale_or_missing_bias`, `accessibility_disabled`, `accessibility_crashed`, `accessibility_not_bound`, `no_accessibility`, `no_shutter_fast_path`, `no_command_complete`, `slow_parse`, `slow_speech_parse`, `slow_speech_access`, `slow_command_access`, `slow_command_shutter`로 분류한다. 사용자가 인식 실패 상황을 모아 보고할 때는 summary/TSV 파일과 실패 trial의 `log_file`을 함께 확인한다.

## 8.1 Owner Voice Gate

소유자 목소리 인증은 오픈소스 `sherpa-onnx` Android 런타임과 3D-Speaker CAM++ speaker verification 모델을 사용한다.

2026-06-22 호출형 정책에서 owner voice의 역할은 바뀌었다. 소유자 프로필은 Jarvis command window 시작 전 준비 상태를 확인하는 조건으로 유지하지만, 기본 어시스턴트 호출로 열린 command window 안의 각 명령은 별도 speaker verification 없이 `자비스` 호출어가 포함된 텍스트 명령으로 처리한다. 이는 idle 마이크 상시 대기를 제거해 배터리 소모를 줄이기 위한 선택이다. 물리 버튼을 누를 수 있는 사람이 command window를 열 수 있으므로, 잠금화면/타인 사용 상황에서 더 강한 보안이 필요하면 다음 단계에서 “명령 음성 자체의 owner verification”을 별도 설계해야 한다.
...
12. Android STT partial 1순위는 진단만 기록하고 실행하지 않는다. 사진 촬영을 포함한 모든 명령은 final 결과에서 확정한다.
...
14. final Android STT 또는 local fallback에서 카메라 세션 명령과 `home` 앱 이동 명령이 확정되면 실행한다.
...
`take_photo`는 실행 지연을 최소화하기 위해 Xiaomi 기본 카메라의 셔터 상대 좌표를 먼저 접근성 제스처로 탭한다. portrait 기준 `x=50%`, `y=88%`이고 gesture duration은 `45ms`다. `dispatchGesture`가 실패한 경우에만 접근성 노드에서 셔터 버튼을 찾는다. 실행 로그는 `shutter_tap_dispatch result=coordinate|node|failed`로 실제 셔터 dispatch 경로를 남긴다.
...
개발 중 기본 설치 방법은 `scripts/jarvis-apk.sh` 또는 Android Studio 직접 설치다.
...
6. Jarvis 앱 실행
7. `시작하기`에서 마이크, 내 목소리, 접근성 필수 설정 완료
8. 필요하면 상태 알림과 배터리 제한 예외 설정
9. `전원 버튼으로 빠르게 호출`에서 Jarvis를 기본 디지털 어시스턴트로 선택
10. HyperOS에서 전원 버튼 long press가 기본 어시스턴트를 실행하도록 설정
11. 전원 버튼 long press 또는 `Jarvis에게 말하기`로 command window 시작

`./scripts/jarvis-apk.sh build debug`는 debug APK와 이식 가능한 `.sha256`을 `dist/`에 만들고, `install debug [serial]`은 SDK/ABI/version을 검사한 뒤 데이터 삭제 없이 `adb install -r`로 설치한다. production release는 저장소 밖 고정 key와 `config/jarvis-release-cert.sha256` 인증서 pin이 모두 있어야 한다. 상세 절차는 [`APK_INSTALLATION.md`](APK_INSTALLATION.md)를 따른다.

APK 수동 설치도 가능하지만, 접근성 서비스는 반드시 사용자가 직접 켜야 한다. Android/HyperOS가 debug APK 또는 수동 설치 APK에 대해 `앱의 액세스가 거부됨`을 표시하면 `설정 > 앱 > 앱 관리 > Jarvis > 오른쪽 상단 메뉴 > 제한된 설정 허용`을 먼저 켠 뒤 접근성 설정에서 Jarvis를 다시 활성화한다. USB 디버깅이 연결된 개발 기기에서는 `enabled_accessibility_services`의 기존 값을 보존한 채 `com.personal.jarvis/com.personal.jarvis.JarvisAccessibilityService`를 추가하고 `accessibility_enabled=1`로 설정해 테스트용 활성화가 가능하다. 단, Android accessibility manager가 Jarvis를 `Crashed services`에 넣은 상태는 secure settings만 다시 써도 해제되지 않을 수 있으므로, 접근성 설정에서 Jarvis를 껐다 켜야 한다. 이 설정이 꺼져 있거나 서비스가 bind되지 않으면 하이퍼아일랜드 overlay와 `전면`/`후면`/`사진 찍어`/`카메라 종료`는 동작하지 않는다.
...
- Repo-local `./gradlew`로 `testDebugUnitTest`, `assembleDebug`, `assembleRelease`, `lintDebug` 실행 성공
- `scripts/jarvis-quality-check.sh`가 단위 테스트, debug/release 빌드, Android lint, release debug-component 부재, debug ADB trampoline/export 계약, unstaged/staged `git diff --check`, `bash -n scripts/*.sh`를 모두 통과
...
- 접근성 서비스가 꺼져 있거나 설정에는 남아 있지만 bind되지 않았으면 `Jarvis에게 말하기`와 기본 어시스턴트 호출이 command window를 시작하지 않고 `시작하기` 복구 안내를 표시함
- 배터리 최적화/앱 정보 설정 화면으로 이동할 수 있음
- 앱 메인 화면의 `지원 명령`에서 지원 명령 수, 예시 문구 수, 모든 `CommandBus` 명령의 대표 문구와 맞춤 발음 수를 확인할 수 있고, 명령 선택 시 사용자 중심 동작과 필요 조건을 확인할 수 있음
- `지원 명령` 상세 화면에서 마이크 권한이 없으면 맞춤 발음 녹음 전 권한 요청이 발생함
- `지원 명령` 상세 화면에서 명령별 3초 맞춤 발음을 한 번씩 녹음하면 WAV/JSON이 앱 private storage에 저장되고 목록 수가 갱신됨
- 같은 명령 샘플을 12개 초과로 녹음하면 오래된 샘플이 자동 삭제되고 최신 12개만 유지됨
- 명령어별 샘플 삭제를 누르면 해당 명령의 샘플만 삭제되고 다른 명령 샘플은 유지됨

### Voice Test

- 앱의 `Jarvis에게 말하기` 버튼 또는 기본 어시스턴트 호출로 `JARVIS LISTENING` Hyper Island overlay가 표시되고 30초 command window가 열린다.
- 위 항목은 Jarvis 접근성 서비스가 켜진 상태에서만 통과해야 한다. 접근성이 꺼져 있으면 overlay가 불가능하므로 command window 시작을 막고 앱 화면으로 안내한다.
- command window 안에서 호출어 없는 `카메라 실행`, `후면`, `전면`, `찍어`, `종료`는 실행되지 않는다.
- command window 안에서 `자비스 카메라 실행`이 기본 카메라 앱을 연다.
- command window 안에서 `자비스 카메라 전면`이 기본 카메라 앱을 전면 카메라 힌트와 함께 열거나 전면으로 전환한다.
- command window 안에서 `자비스 카메라 후면`이 기본 카메라 앱을 후면 카메라 힌트와 함께 열거나 후면으로 전환한다.
- command window 안에서 `자비스 사진 찍어`가 현재 카메라 화면에서 셔터를 누른다.
- command window 안에서 `자비스 카메라 종료`가 홈으로 이동한다.
- command window 안에서 `자비스 화면 켜`가 꺼진 화면을 깨워 잠금화면을 표시한다.
- command window 안에서 `자비스 화면 꺼`가 접근성 잠금화면 전역 액션으로 기기를 잠근다.
- command window 안에서 `자비스 잠들어`와 `자비스 멈춰`가 command window를 닫고 음성 서비스를 종료한다.
...
- Android STT 실패 후 local ASR이 텍스트 명령을 파싱하지 못하고 local ASR 텍스트가 비었거나 호출어를 포함하며, 같은 명령의 서로 다른 유효 음성 exemplar 2개가 모두 거리/길이 기준을 통과하고 다른 유효 consensus 명령과의 margin도 확보되면 `local_complete endpoint=voice_sample_match`와 `command_parsed source=local` 경로로 명령이 실행된다.
...
- 기본 어시스턴트 호출 또는 `Jarvis에게 말하기` 후 컷아웃 영역 Hyper Island pill `JARVIS [camera hole] LISTENING`, 짧은 확인음 1회, 짧은 진동 1회가 발생한다.
...
- command window 종료 상태에서는 overlay가 사라진다.
...
- `시작하기 → 내 목소리 등록` 후 6초 녹음 진행률이 20% 단위로 올라간다.
- 음성 등록 또는 맞춤 발음 녹음 중 화면을 벗어나면 녹음과 예약 callback이 중지된다.
- 등록 완료 시 2개 이상 8개 이하의 owner embedding이 저장된다.
- owner embedding 묶음과 `profile_phrase_id=jarvis_activation_v3`가 저장된 상태에서만 command window 시작을 허용한다.
- 현재 호출형 정책에서는 command window 안의 각 명령에 별도 owner voice verification을 적용하지 않는다.
- 더 강한 보안이 필요하면 command audio 자체의 owner verification을 별도 테스트 항목으로 추가한다.

### Boot/Assistant Invocation Test

- `MY_PACKAGE_REPLACED` 또는 재부팅 후 `Jarvis 준비됨` 알림이 표시된다.
- 알림을 누르면 `JarvisAssistantActivity`를 통해 30초 command window가 열린다.
- Jarvis가 기본 디지털 어시스턴트로 선택된 상태에서 전원 버튼 long press가 Jarvis command window를 연다.
- 접근성 서비스만 살아 있고 `JarvisVoiceService`가 없을 때 watchdog이 음성 서비스를 자동 재시작하지 않는다.
- 저장된 owner embedding이 2개 미만인 구버전 프로필이면 `JarvisVoiceServiceStarter`가 command window 시작을 차단한다.
- Android 정책상 `BOOT_COMPLETED`에서 microphone foreground service가 직접 시작되지 않는다.
- command window 종료 후 foreground notification과 마이크 privacy indicator가 사라지는지 확인한다.
...
- 명령어별 음성 샘플 매칭은 ASR 실패 보조 휴리스틱이다. 서로 비슷한 명령을 비슷한 톤으로 녹음하면 후보가 애매해져 실행되지 않거나, threshold가 느슨하면 오탐 가능성이 생긴다. 현재는 명령당 최소 2개 샘플, 명령어당 최신 12개 보관, 호출어 없는 텍스트 fallback 차단, 거리 threshold, 길이 비율, 다음 유효 consensus 후보와의 margin으로 보수적으로 제한한다. threshold 조정은 `command-recognition-attempts/` replay와 저장 샘플 진단 로그를 먼저 확인한 뒤 진행한다.
...
### 2026-07-18 출시 UX·개발자 메뉴·APK 설치 흐름

목표는 기존 기능 테스트형 메인을 실제 사용자 흐름으로 재구성하고, 테스트 기능을 debug 전용 화면으로 격리하며, APK를 검증 가능한 방식으로 만들고 설치하는 것이다.

핵심 변경:

1. `MainActivity`를 준비 상태 hero, 단일 `Jarvis에게 말하기` CTA, 실시간 `듣는 중/처리 중/완료/실패`, 대표 명령과 빠른 메뉴 중심으로 재설계했다.
2. `JarvisSetupStatus`와 `SetupActivity`를 추가해 마이크 → 소유자 목소리 → 접근성 필수 설정을 순서대로 안내하고, 기본 어시스턴트, 알림, 배터리, HyperOS 앱 설정은 권장 항목으로 분리했다.
3. 권한 승인 뒤 소유자 등록을 자동 재개한다. 등록·명령 샘플 recorder는 generation token으로 중단된 worker와 늦은 callback을 폐기하고, 화면 이탈·dialog dismiss에서 예약 작업과 녹음을 중단한다. 검증을 통과한 등록 녹음만 atomic 교체하며, 소유자 프로필 삭제는 확인 dialog와 실행 중 서비스 정지 후 embedding, 원본 등록 WAV, matcher cache를 함께 제거한다.
4. `JarvisStateBus`를 메인에서 직접 수신해 비동기 서비스 시작 직후 상태가 `꺼짐`으로 깜박이던 문제를 없앴다. `CommandBus`도 프로세스 내부 전용으로 바꿔 API 26~32의 외부 명령 broadcast 주입을 제거했다.
5. 메인의 카메라/전후면/셔터/화면 테스트 6개를 `src/debug`의 `JarvisDeveloperMenuActivity`로 옮겼다. release manifest에는 이 메뉴와 기존 debug trampoline이 포함되지 않는다.
6. 사용자 명령 상세에서 final STT, fast-partial 정책, command id를 제거하고, 맞춤 샘플을 `내 발음으로 인식 개선` 기능으로 표현했다. 명령 목록에 명시적 뒤로가기 진입도 추가했다.
7. `JarvisLogSanitizer`가 release latency detail의 `text`, `first`, `candidates`, `photo` 중 첫 민감 필드부터 이후 값을 제거하고, debug APK에서만 실제 인식 진단 문자열을 유지한다. key 모양 문자열을 발화에 넣어도 우회할 수 없는 단위 테스트를 추가했다.
8. 앱 버전을 `1.0.0(2)`로 올리고 ASR 모델 다운로드에 SHA-256 검증을 추가했다. release 서명은 저장소 밖 key를 Gradle property, 환경 변수 또는 Git 제외 `keystore.properties`로 받고, 공개 인증서 fingerprint를 고정한다.
9. `scripts/jarvis-apk.sh`가 debug/release 빌드, signingReport 사전 검증, APK 서명/package/arm64-only/SDK/manifest/16 KiB alignment 검증, versionName·versionCode 배포 파일과 이식 가능한 SHA-256 생성, 데이터 삭제 없는 `adb install -r`를 담당한다. release 업데이트는 동일하거나 낮은 versionCode를 거절한다.
10. 메인 notice는 외부 문자열 대신 allowlist code만 받고, 설정 완료 시 자동 해제한다. Setup과 명령 목록은 스크롤 위치, 48dp touch target, 컷아웃 4방향 inset, 색 대비와 TalkBack 진행률 throttle을 반영한다.
11. `COMMAND_PROCESSING`은 최소 350ms 표시한다. `카메라 실행 후 촬영`은 1.5초 뒤 셔터 제스처의 실제 완료/취소 callback이 올 때까지, 전·후면 카메라 명령은 전환 제스처 완료 후 목표 렌즈를 다시 확인할 때까지 처리 중을 유지한다. dispatch 또는 렌즈 확인 실패는 `COMMAND_FAILED`, 성공은 `명령 전달 완료`로 구분한다. 명령 실행 generation, 취소 가능한 결과/닫기/timeout runnable과 5초 watchdog으로 이전 명령 callback이나 기존 30초 window timeout이 새 세션을 닫지 못하게 한다. in-flight reservation과 완료 시점 cooldown은 같은 명령의 겹친 실행을 차단한다.
12. local streaming ASR partial도 Android STT와 동일하게 진단에만 쓰고 trailing-silence final decode에서 실행한다. 명시적 부정 final은 맞춤 음성 샘플 fallback에도 들어가지 않는다.
13. 최종 검증은 108개 debug 단위 테스트, debug/release assemble, lintDebug, release debug-component 부재, debug 개발자 메뉴/ADB trampoline·내부 명령·백업 제외 계약, APK 자체 검증과 shell/diff check로 구성한다. 실제 기기 설치는 사용자 요청에 따라 보류한다.

APK 사용법과 release 서명 체크리스트는 [`APK_INSTALLATION.md`](APK_INSTALLATION.md)에 정리한다. debug APK는 개발·실기기 검증용이며 사용자 배포용으로 사용하지 않는다.

### 2026-07-18 음성 경로 정확도·속도·구조 리뷰

우선순위는 음성 인식 정확도, 속도, 코드 구조 순서로 두었다. 상세 근거, 반영 내역, 회귀 테스트와 잔여 위험은 [`CODE_REVIEW_2026-07-18.md`](CODE_REVIEW_2026-07-18.md)에 정리했다.

핵심 변경:

1. Android partial은 1순위 진단만 남기고 실행하지 않으며, 모든 명령을 final에서 확정한다.
2. final 후보 중 명시적인 부정/취소가 하나라도 있으면 전체를 거절한다. 그 외에는 전체 strict parse 후 `take_photo` 하나에만 clipped final fallback을 적용한다.
3. 호출어를 문장 시작으로 제한하고 약한 `서비스`/`잡스` 보정은 사진 문맥에서만 허용했다. 후보 안의 명시적 부정/취소 marker와 `홈페이지`/`백업` substring 오탐도 거절한다.
4. final callback 2.5초 watchdog, 새 command window의 예약 `stopSelf()` 취소, local session과 Android recognizer generation token을 추가했다.
5. 명령 음성 샘플은 서로 다른 유효 exemplar 2개가 모두 일치해야 승인한다.
6. local ASR asset 확인을 캐시하고 command recognizer만 기본 예열한다. legacy activation recognizer는 실제 요청까지 lazy load한다.

검증은 parser/selector/sample consensus/session generation 단위 테스트, 전체 `testDebugUnitTest`, debug APK build, lint, shell syntax와 diff check 순서로 수행한다. 실기기 반복 발화 결과가 없는 상태에서는 실제 recall/P95 개선 수치를 확정하지 않는다.
...
3. 당시 `JarvisAccessibilityService`의 동적 receiver 등록 lint error를 정리했다. 이 receiver는 2026-07-18 프로세스 내부 `CommandBus` 전환으로 완전히 제거되어 현재 코드에는 broadcast 명령 경로가 없다.
...
6. `LocalCommandRecognizer.listenForCommand()`가 final local ASR 텍스트를 명령으로 파싱하지 못한 경우에만 저장 샘플 fallback을 시도한다. 오탐 방지를 위해 local ASR 텍스트가 비어 있지 않으면 호출어가 포함되어야 하고, 같은 명령에 샘플이 2개 이상이어야 하며, best distance, 길이 비율, next eligible consensus command distance margin 기준을 통과해야 `voice_sample_match` endpoint로 명령을 반환한다.
...
- 2026-07-18 이후 현재 정책에서는 이 helper를 partial 1순위 진단과 final clipped-photo 복구에 사용하되, command partial 자체는 실행하지 않는다.
...
- `partial_command_deferred`가 거의 없으면 Android STT partial callback이 늦거나 누락된 상황으로 보고, 실행 정책을 넓히지 말고 final 후보 복구와 final latency를 우선 본다.
